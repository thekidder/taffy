...
int SORE_Font::FontInfo::LoadFont(const char* fontName)
{
	...
	if ((err=FT_New_Memory_Face( library, fontInfo, size, 0, &face ))!=0) 
	{
		...
		ENGINE_LOG(SORE_Logging::LVL_ERROR, "Font load failed: Freetype error code %d", err);
#ifdef DEBUG
		//std::cout << ft_errors[err] << std::endl;
		ENGINE_LOG(SORE_Logging::LVL_ERROR, "Freetype error: %s", ft_errors[err]);
#endif
		return FONT_LOAD_FAILED;
	}
	...
	while((error=glGetError())!=GL_NO_ERROR)
	{
		...
		ENGINE_LOG(SORE_Logging::LVL_ERROR, "GL Error: %d", error);
		delete[] fontPath;
...
SORE_Font::font_ref SORE_Font::LoadFont(const char* font, unsigned int h)
{
	...
	if((err=newFont.LoadFont(font))!=0)
	{
		...
		ENGINE_LOG(SORE_Logging::LVL_ERROR, "Failed to load font: error code %d", err);
		return 0;
	}
