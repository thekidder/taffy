...
# 
# Changelog: 
# (Osbios - http://www.sfml-dev.org/forum/viewtopic.php?t=753&postdays=0&postorder=asc&start=0)
...
#            - You can chose wich static/shared versions of the librarys you want to link to 
# (Adam Kidder - <thekidder@gmail.com> )
# 2011-06-11 - Remove switch for debug/non-debug: links to both depending on CMAKE_BUILD_TYPE
#            - Cleaned up some typos and documentation
...
FOREACH(MODE ${SFML_MODE}) 
  ...
  FOREACH(COMPONENT ${SFML_COMPONENTS})       
    ...
    if(WIN32 AND NOT(_STA))
      # look for DLLs for ease of copying
      FIND_FILE(SFML_${UPPERCOMPONENT}_DLL${MODE}
	sfml-${LOWERCOMPONENT}${_DBG}.dll
	PATH_SUFFIXES bin lib
	PATHS ${SFML_LIBRARY_SEARCH_DIR} 
	)
    ENDIF(WIN32 AND NOT(_STA))
  ENDFOREACH(COMPONENT) 
...
SET(DEBUG_MODES "DEBUG" "NONDEBUG")
FOREACH(DEBUG_MODE ${DEBUG_MODES})
  FOREACH(COMPONENT ${SFML_FIND_COMPONENTS}) 
    SET( MODULE_NAME SFML_${COMPONENT}_LIBRARY${SHARED_MODE}_${DEBUG_MODE} ) 
    SET( DLL_NAME    SFML_${COMPONENT}_DLL${SHARED_MODE}_${DEBUG_MODE} ) 
    STRING(TOUPPER ${MODULE_NAME} MODULE_NAME) 
    STRING(TOUPPER ${DLL_NAME} DLL_NAME) 
    
    IF(NOT ${MODULE_NAME}) 
      SET(SFML_FOUND "NO") 
      MESSAGE("-- SFML: Could not locate : ${MODULE_NAME}") 
    ELSE(NOT ${MODULE_NAME}) 
      LIST(APPEND SFML_LIBRARY_${DEBUG_MODE} ${${MODULE_NAME}}) 
      LIST(APPEND SFML_DLL_${DEBUG_MODE} ${${DLL_NAME}})
...
IF (SFML_FOUND)
  MESSAGE(STATUS "SFML was found.")
  MESSAGE (STATUS "  include path: ${SFML_INCLUDE_DIR}")
  MESSAGE (STATUS "  library path: ${SFML_LIBRARY}")
  IF(WIN32 AND NOT(SFML_STATIC))
    MESSAGE (STATUS "  DLL path: ${SFML_DLL}")
  ENDIF()
ENDIF()
